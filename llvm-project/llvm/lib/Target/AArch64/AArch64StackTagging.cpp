...
public:
  ...
  void tagAlloca_analogue(Value *taggedPtr, Instruction *InsertBefore, uint64_t Size);
...
void AArch64StackTagging::tagAlloca_analogue(Value *taggedPtr, Instruction *InsertBefore, uint64_t Size){
#if STACK_POOLING == 0
  IRBuilder<> builder(InsertBefore);
  LLVMContext &C = taggedPtr->getContext();
  Value* sz = builder.getInt64(Size);
  InlineAsm *IA = InlineAsm::get(
              FunctionType::get(llvm::Type::getVoidTy(C), {taggedPtr->getType(), sz->getType()}, false),
              StringRef("mov x2, $0\n mov x3, $1\n mov x17, $0\n cbz $1, 2f\n 1:\n mov x16, $0\n lsr x16, x16, #56\n and x16, x16, #0xFUL\n strb w16, [x17, #0x0]\n add $0, $0, #16\n sub $1, $1, #16\n add x17, x17, 1\n cbnz    $1, 1b\n 2:\n mov $0, x2\n mov $1, x3\n "),
              ...
              InlineAsm::AD_ATT,
              /*canThrow*/ false);
   std::vector<llvm::Value*> args = { taggedPtr, sz };
...
void AArch64StackTagging::tagAlloca(AllocaInst *AI, Instruction *InsertBefore,
...
#if 0
   Value* tag = builder.CreateShl(builder.getInt64(7), 56);
   Value* allocaInt = builder.CreatePtrToInt(AI, tag->getType());
   Value* taggedPtrInt = builder.CreateOr(allocaInt, tag);
   Value* taggedPtr = builder.CreateIntToPtr(taggedPtrInt, AI->getType());

  // fcg: this replace breaks some assumptions here. but its okay the tag is still set for the analogue
  // just further ptr uses do not have the tag but thats fine for emulation

   AI->replaceUsesWithIf(taggedPtr, [&](const Use &u){
  ...
  Value* sz = builder.getInt64(Size);
  InlineAsm *IA = InlineAsm::get(
              FunctionType::get(llvm::Type::getVoidTy(C), {AI->getType() /*taggedPtr->getType()*/, sz->getType()}, false),
              StringRef("mov x2, $0\n mov x3, $1\n mov x17, $0\n cbz $1, 2f\n 1:\n mov x16, $0\n lsr x16, x16, #56\n and x16, x16, #0xFUL\n strb w16, [x17, #0x0]\n add $0, $0, #16\n sub $1, $1, #16\n add x17, x17, 1\n cbnz    $1, 1b\n 2:\n mov $0, x2\n mov $1, x3\n "),
              StringRef("r,r,~{x16},~{x17},~{x2},~{x3},~{memory}"),
              /*hasSideEffects=*/ true,
              /*isAlignStack*/ false,
              ...
              /*canThrow*/ false);
   //std::vector<llvm::Value*> args = { taggedPtr, sz };
  std::vector<llvm::Value*> args = { builder.CreatePointerCast(AI, builder.getInt8PtrTy()), sz };
  
  builder.CreateCall(IA, args);
#endif


#if __ORIGINAL_MTE_CODE == 1
  auto SetTagZeroFunc =
...
void AArch64StackTagging::untagAlloca(AllocaInst *AI, Instruction *InsertBefore,
  ...
  Value* sz = builder.getInt64(Size);
  InlineAsm *IA = InlineAsm::get(
              FunctionType::get(llvm::Type::getVoidTy(C), {AI->getType(), sz->getType()}, false),
              StringRef("mov x2, $0\n mov x3, $1\n mov x17, $0\n cbz $1, 2f\n 1:\n mov x16, $0\n lsr x16, x16, #56\n and x16, x16, #0xFUL\n strb w16, [x17, #0x0]\n add $0, $0, #16\n sub $1, $1, #16\n add x17, x17, 1\n cbnz    $1, 1b\n 2:\n mov $0, x2\n mov $1, x3\n "),
              StringRef("r,r,~{x16},~{x17},~{x2},~{x3},~{memory}"),
              /*hasSideEffects=*/ true,
              /*isAlignStack*/ false,
...
bool AArch64StackTagging::runOnFunction(Function &Fn) {
  ...
  errs() << "[MemTagSan] running on Function!\n";
  ...
  if (SInfo.AllocasToInstrument.empty())
    return false;
#if 0
  if(Fn.getName() == "search"){
      errs() << "Dumping for F: " << Fn.getName() << "\n";
      for (auto &I : SInfo.AllocasToInstrument) {
         memtag::AllocaInfo &Info = I.second;
         AllocaInst *AI = Info.AI;
         errs() << "Instrumenting: " << *AI << "\n";
      }
  }
#endif
  ...
  // [[1]] fcg: insertBaseTaggedPointer has a CreateCall to aarch64_irg_sp instruction
  ...
  for (auto &I : SInfo.AllocasToInstrument) {
    ...
    // [[2]] fcg: TagP results in ADDG/SUBP (pointer arithmetic and stack tagging)
    // I think this code makes sure that allocas receive a Tag in ptr
    ...
    // fcg: tag pointer here
    // crash
    Value* allocaInt = IRB.CreatePtrToInt(Info.AI, IRB.getInt64Ty());
    Value* taggedPtrInt = IRB.CreateOr(allocaInt, IRB.getInt64((uintptr_t)0x700000000000000));
    Value* taggedPtr = IRB.CreateIntToPtr(taggedPtrInt, Info.AI->getType());
    //for(auto U : Info.AI->users()){
    //    if(auto I = dyn_cast<Instruction>(U)){
    //        if(I != dyn_cast<Instruction>(allocaInt)){
    //            errs() << *Info.AI << " USED BY: " << *I << "\n";
    //        }
    //    }
    //}
//    errs() << "The Alloca to Replace: " << *AI << " by taggedptr: " << *taggedPtr << "\n";
    Info.AI->replaceUsesWithIf(taggedPtr, [&](const Use &u){
  //      if(u.getUser() == allocaInt) return false;

        // 470.lbm a CallInst is causing the crash
        //if(auto I = dyn_cast<CallInst>(u.getUser())) return false;

    //    if(auto I = dyn_cast<Instruction>(u.getUser())){
      //      if(IntrinsicInst *II = dyn_cast<IntrinsicInst>(I)){
        //        return false;
          //  }
            //errs() << "Replace Alloca Use U Instruction: " << *I << "\n";
       //}
    ...
    if (StandardLifetime) {
      ...
      // [[3]] fcg: Tag Alloca here, some untags below
#if 0
      tagAlloca(AI, Start->getNextNode(), Start->getArgOperand(1), Size);
#endif
      tagAlloca_analogue(taggedPtr, Start->getNextNode(), Size);


      auto TagEnd = [&](Instruction *Node) {
#if 0
        untagAlloca(AI, Node, Size);
#endif
        tagAlloca_analogue(taggedPtr, Node, Size);
      };
      if (!DT || !PDT ||
      ...
      // [[4]] fcg: TagPCall used here
      uint64_t Size = *Info.AI->getAllocationSize(*DL);
      // fcg: we dont need Ptr for now, we apply | 0x7 << 56 tag ourselves
      Value *Ptr = nullptr; //IRB.CreatePointerCast(TagPCall, IRB.getInt8PtrTy());
#if 0
      tagAlloca(AI, &*IRB.GetInsertPoint(), Ptr, Size);
#endif
      tagAlloca_analogue(taggedPtr, &*IRB.GetInsertPoint(), Size);

      for (auto *RI : SInfo.RetVec) {
#if 0
        untagAlloca(AI, RI, Size);
#endif
        tagAlloca_analogue(taggedPtr, RI, Size);
      }
...
Instruction *AArch64StackTagging::insertBaseTaggedPointer(
  ...
  Base->setName("basetag");
#if TAG_ZERO_WORKAROUND == 1
  // remove the random tag, but we keep IRG for the overhead...
  Value *BaseAsInt = IRB.CreatePtrToInt(Base, IRB.getInt64Ty());
  Value *UntagPtrAsInt = IRB.CreateAnd(BaseAsInt, ConstantInt::get(IRB.getInt64Ty(), (uint64_t)0xffffffffffffff));
  Value *UntagPtr = IRB.CreateIntToPtr(UntagPtrAsInt, Base->getType());
  IRB.CreateCall(SetTagFunc, {UntagPtr, ConstantInt::get(IRB.getInt64Ty(), 16)});
  return UntagPtr;
#else
  return Base;
#endif
...
// FIXME: check for MTE extension
bool AArch64StackTagging::runOnFunction(Function &Fn) {
    //errs() << "[MemTagSan] running on Fn " << Fn.getName() << "\n";
  if (!Fn.hasFnAttribute(Attribute::SanitizeMemTag))
    return false;
  ...
  memtag::StackInfo &SInfo = SIB.get();
#if 0
   errs() << "[MemTagSan] Allocas To Instrument:\n";
  for (auto &I : SInfo.AllocasToInstrument) {
    memtag::AllocaInfo &Info = I.second;
    AllocaInst *AI = Info.AI;
    errs() << "[MemTagSan]   " << *AI << "\n";
...
#if TAG_ZERO_WORKAROUND == 1
  Value *Base = insertBaseTaggedPointer(SInfo.AllocasToInstrument, DT);
#else
  Instruction *Base = insertBaseTaggedPointer(SInfo.AllocasToInstrument, DT);
#endif
